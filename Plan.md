...
- [ ] Настроить `config/settings.py` (CORS, DRF, JWT, Celery)
- [ ] Настроить роутинг (`urls.py`)

### Этап 3: Модель User + Auth (2-3 часа)
- [ ] Создать модель `User` с полем `telegram_chat_id`
- [ ] Создать сериализаторы для регистрации/авторизации
- [ ] Реализовать ViewSets для `/api/register`, `/api/token`
- [ ] Тестирование регистрации через Postman/Swagger
